...
#
# Writes: dist/Armored-Core-Recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix Armored-Core-Recomp \
  --exe-name Armored_Core_Recompiled \
  --display-name "Armored Core Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Armored Core disc" \
